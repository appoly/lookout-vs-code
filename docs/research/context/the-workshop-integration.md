...
**Implication:** Parful should integrate through those durable files and their metadata, not by scraping terminal prose or assuming Claude and Codex have identical commands. Artifact discovery must include agent worktrees even when those worktrees are outside the opened VS Code workspace.
...
**Implication:** Parful's agent-first worktree grouping is aligned with the workflow. Fleet manifests, child agents, worktree/branch identity, PR results, and preserved failed worktrees are promising integration points. Attribution must remain worktree-based unless the workflow provides stronger provenance.
...
**Implication:** Parful can detect a compatible installation without owning it, show the detected runtime/version/scope, and link to separate installation guidance. It should never silently install, update, or bundle the skills. The Workshop remains an independently released optional compatibility pack.
...
4. **Fleet cockpit:** parse `docs/fleet/*.md`, associate rows with Parful agents/worktrees/branches, and show dependency, blocked, PR, verification, and review state without claiming unsupported provenance.
...
Parful will recommend The Workshop for maximum compatibility, but the extension will not vendor the skill files. The skills, installer, versions, and update lifecycle stay in their own distribution. Parful's core experience must continue to work with ordinary Codex, Claude Code, and custom terminal agents when no Workshop installation is present.
