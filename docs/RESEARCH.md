...
Claude sessions launched directly by Lookout receive session-only `UserPromptSubmit`, `Notification`, `SubagentStart`, `SubagentStop`, `Stop`, `StopFailure`, and allow-listed `PreToolUse`/`PostToolUse` hooks through a temporary per-session `--settings` file. These report working, permission attention, delegated-agent IDs, foreground stop, failure, shell commands, and MCP tool identifiers without modifying global Claude settings. Current hook input identifies agent ID/type but does not say whether the child was foregrounded or backgrounded, so Lookout uses the honest umbrella term “delegated.”

The Codex CLI's external `notify` setting currently emits `agent-turn-complete`. Direct Codex sessions receive command-line-only lifecycle hooks for prompt, authorization-check, delegated-agent, stop, and allow-listed tool-use (`PreToolUse`/`PostToolUse`) events plus that notifier as a fallback. Verified against `codex-cli 0.144.1`: the shell tool's canonical hook name is `Bash` and the payload matches Claude's (`tool_name`, `tool_input.command`, `tool_use_id`), so activity tracking shares one extraction path across both providers. The matcher also accepts `codex_apps.*` and `mcp__*` tool names. Lookout retains only that bounded identifier for MCP activity and never forwards its arguments or response; shell-result capture remains a separate explicit opt-in. Codex's `PermissionRequest` hook runs before automatic guardian or user review and has no later approval result, so it cannot honestly mean that a human is blocked. Codex requires non-managed hooks to be reviewed through `/hooks`; Lookout does not bypass that security boundary. Commands with shell operators or wrappers are not rewritten, and explicit hook/notifier overrides are preserved.
...
- Session records contain friendly metadata and working-folder state—not raw
  terminal output or prompts. In-flight shell labels and MCP tool identifiers
  are memory-only and are cleared at persistence boundaries.
